...
exports.getGroupData = function getGroupData(session, group_id, callback) {
    MongoClient.connect(MONGO_CONNECT_URI, function(err, db) {
        if (err) {
            console.log(err);
            return;
    ...
    groupme.getGroupInfo(session, group_id, function(group) { // get the group info
        ...
        MongoClient.connect(MONGO_CONNECT_URI, function(err, db) {
            ...
            collection.insert(group, {
                "safe": true
            }, function(err, object) { // add the group to the DB
                callback();
...
// delete the group, then add it back
exports.updateGroupInDB = function updateGroupInDB(session, group_id, callback) {
    MongoClient.connect(MONGO_CONNECT_URI, function(err, db) {
        if (err) {
            console.log(err);
            return;
